...
export function useWaveSurfer(rawAudio: File, rawAudioDuration: number) {
  ...
  const initializeWaveSurfer = async () => {
    ...
    wavesurfer.value.on('ready', () => {
      ...
      regionInstance.on('update-end', () => {
        updateExportRegion({ start: regionInstance.start, end: regionInstance.end });
      });
  ...
  const updateExportRegion = (updatedRegion: { start: number; end: number }) => {
  ...
  const adjustStartTime = (delta: number) => {
    const newStart = Math.max(0, Math.min(region.value[0] + delta, region.value[1] - 0.1));
  ...
  const adjustEndTime = (delta: number) => {
    const newEnd = Math.max(region.value[0] + 0.1, Math.min(region.value[1] + delta, rawAudioDuration));
    const regions = regionsPlugin.value.getRegions();
    if (regions.length > 0) {
      const r = regions[0];
